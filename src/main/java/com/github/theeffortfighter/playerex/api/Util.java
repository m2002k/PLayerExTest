...
public class Util {
	 ...
	 * @param crnt double Current Value (Input 1).
	 * @param val double Adding/Subtracting Value (Input 2).
	 * @param limt double Limit.
	 * @return double Diminishing returns output.
	 */
	public static double dim(final double crnt, final double val, final double limt) {
		if(limt <= 0D) return 0D;
		if(crnt >= 0D && val >= 0D) return limt * (1.0D - ((1.0D - (crnt / limt)) * (1.0D - (val / limt))));
		if(crnt >= 0D && val <= 0D) return limt * (1.0D - ((1.0D - (crnt / limt)) / (1.0D - ((-val) / limt))));
		if(crnt <= 0D && val >= 0D) return limt * (1.0D - ((1.0D - (val / limt)) / (1.0D - ((-crnt) / limt))));
